...
export async function executeNode(nodeId: string, execId: string): Promise<void> {
  ...
  const executionKind = getNodeExecutionKind(el);

  if (executionKind === 'blockedDraft') {
  ...
  if (executionKind === 'nonGenerative') {
    store.updateNodeStatus(nodeId, 'success');
    return;
  }

  if (executionKind === 'existingOutput') {
    store.updateNodeStatus(nodeId, 'success');
    return;
  }
  ...
  if (executionKind === 'mediaGeneration') {
