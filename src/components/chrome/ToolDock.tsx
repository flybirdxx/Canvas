    ...
    <div className="anim-fade-in" style={{ position:'absolute', bottom:28, left:'50%', transform:'translateX(-50%)', display:'flex', alignItems:'center', gap:4, padding:6, background:'var(--bg-2)', borderRadius:'99px', boxShadow:'var(--shadow-ink-2)', zIndex:30, pointerEvents:'auto' }}>
      ...
      <div style={{ position:'relative' }}>
        <button type="button" onClick={() => setMenuOpen(v => !v)}
          style={{ width:38, height:38, display:'flex', alignItems:'center', justifyContent:'center', border:'none', borderRadius:'50%', background:menuOpen?'var(--bg-3)':'transparent', color:'var(--ink-0)', cursor:'pointer', transition:'all 120ms ease' }}
          title="Add">
        ...
        {menuOpen && (<>
          ...
          <div className="anim-pop" style={{ position:'absolute', bottom:'calc(100% + 12px)', left:0, width:260, padding:8, background:'var(--bg-2)', borderRadius:'12px', boxShadow:'var(--shadow-ink-3)', zIndex:50, display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <Pick onClick={() => { onCreate('text'); setMenuOpen(false); }} icon={<Type size={15} strokeWidth={1.6} />} label="Text" desc="Text / prompt / note" hotkey="T" />
            <Pick onClick={() => { onCreate('image'); setMenuOpen(false); }} icon={<ImageIcon size={15} strokeWidth={1.6} />} label="Image" desc="Generate / upload" hotkey="I" />
            <Pick onClick={() => { onCreate('video'); setMenuOpen(false); }} icon={<Video size={15} strokeWidth={1.6} />} label="Video" desc="Text/image to video" hotkey="V" />
            <Pick onClick={() => { onCreate('audio'); setMenuOpen(false); }} icon={<Music size={15} strokeWidth={1.6} />} label="Audio" desc="Voice / BGM" hotkey="A" />
            <Pick onClick={() => { onCreate('sticky'); setMenuOpen(false); }} icon={<StickyNote size={15} strokeWidth={1.6} />} label="Sticky" desc="Quick note" hotkey="S" />
            <Pick onClick={() => { onCreate('rectangle'); setMenuOpen(false); }} icon={<Square size={15} strokeWidth={1.6} />} label="Shape" desc="Rectangle / circle" hotkey="R" />
            <Pick onClick={() => { ref.current?.click(); setMenuOpen(false); }} icon={<FileUp size={15} strokeWidth={1.6} />} label="File" desc="Any format" hotkey="U" />
